...
def report():
    ...
    with h5py.File(DATASET, "r") as f:
        ...
        for name in demos:
            ...
            # payload body center vs meeting offset:
            wrist_to_payload = np.linalg.norm(meeting - payload)
            ...
            rows.append(
                dict(
                    ...
                    meeting=meeting,
                    d1=d1,
                    wrist_to_payload=wrist_to_payload,
                    reach1_xy=reach1_xy,
    ...
    print("\nscalars:")
    stats("d1", None)
    stats("wrist_to_payload", None)
    stats("reach1_xy", None)
    stats("reach0_head_xy", None)

    print("\n--- payload body vs meeting (wrist) relationship ---")
    head = arr["payload"]
    meet = arr["meeting"]
    delta = head - meet
    ...
    print(f"  n payloads with |x|>0.35: {clamp_hit.sum()} / {len(px)}")
    if clamp_hit.any():
