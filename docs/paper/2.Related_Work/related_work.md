...
The three lines above converge on an unexplored intersection. Mixture-of-experts policies provide modular architectures whose routing can, in principle, reflect the phase structure of a task. Trajectory segmentation methods can discover that phase structure from demonstrations. Initialization is known to have persistent effects on neural network optimization and on expert specialization in MoE models. Yet the specific question — whether using demonstration-derived trajectory structure to initialize the routing partition of an MoE policy measurably changes the organization of the learned experts, and whether that change predicts task performance — has not been directly tested.
