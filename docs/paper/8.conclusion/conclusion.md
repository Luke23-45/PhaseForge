...
The answer to the second question is no, or at least not uniformly. The topology-initialized policy leads on Can (76% vs. 63% for BC) but trails on Square (27% vs. 35% for the softmax control), where the most phase-aligned prototype partition produces the lowest success rate. A more structured routing partition does not reliably produce a better controller.
